...
@section('content')
    @component('components.breadcrumb')
        ...
        @endslot
        @slot('li_1')
            <a href="{{ route('psychologist.appointments.show', $prescription->appointment) }}">Appointment Details</a>
        @endslot
        @slot('li_2')
            Edit Prescription
        @endslot
                ...
                <div class="col-lg-8 col-xl-9">
                    <div class="dashboard-header">
                        <div class="header-back">
                            <a href="{{ route('psychologist.appointments.show', $prescription->appointment) }}" class="back-arrow">
                                <i class="fa-solid fa-arrow-left"></i>
                            </a>
                            <h3>Edit Prescription</h3>
                        </div>
                        <p class="text-muted">Update prescription or therapy notes. Changes will be immediately visible to the patient.</p>
                    </div>
                    ...
                    <div class="card mb-4">
                        ...
                        <div class="card-body">
                            <div class="row">
                                <div class="col-md-6">
                                    <p><strong>Patient Name:</strong> {{ $prescription->appointment->patient->user->name }}</p>
                                    <p><strong>Email:</strong> {{ $prescription->appointment->patient->user->email }}</p>
                                </div>
                                <div class="col-md-6">
                                    <p><strong>Appointment Date:</strong> {{ $prescription->appointment->appointment_date->format('M d, Y') }}</p>
                                    <p><strong>Prescription Created:</strong> {{ $prescription->created_at->format('M d, Y h:i A') }}</p>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="card">
                        ...
                        <div class="card-body">
                            <form action="{{ route('psychologist.prescriptions.update', $prescription) }}" method="POST">
                                @csrf
                                @method('PUT')
                                ...
                                <div class="mb-4">
                                    ...
                                    >{{ old('therapy_plan', $prescription->therapy_plan) }}</textarea>
                                    @error('therapy_plan')
                                ...
                                <div class="alert alert-info">
                                    ...
                                    <strong>Note:</strong> Any changes you make will be immediately visible to the patient through their dashboard and appointment details.
                                </div>

                                <div class="d-flex justify-content-between">
                                    <a href="{{ route('psychologist.appointments.show', $prescription->appointment) }}" class="btn btn-light">
                                        <i class="fa-solid fa-times me-2"></i>Cancel
                                    </a>
                                    <button type="submit" class="btn btn-primary">
                                        <i class="fa-solid fa-save me-2"></i>Update Prescription
                                    </button>
                                </div>
                            </form>
                        </div>
